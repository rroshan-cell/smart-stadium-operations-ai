...
def test_context_manager_basic_updates(context_manager):
    """Verify update_state and update_telemetry work correctly."""

    initial_time = datetime.fromisoformat(
        context_manager.shared_state["last_update"]
    )
    ...
    # Timestamp should never move backwards.
    # Equal timestamps are acceptable on systems with coarse clock resolution.
...
def test_context_manager_incident_lifecycle(context_manager):
    ...
    context_manager.assign_incident_ownership(
        "INC-1",
        "MaintenanceAgent",
    )

    assert (
        context_manager.shared_state["incident_ownership"]["INC-1"]
        == "MaintenanceAgent"
    )

    assert (
        context_manager.shared_state["active_incidents"][0]["owner"]
        == "MaintenanceAgent"
    )

    context_manager.resolve_incident(
        "INC-1",
        "Backups stabilized and generator online",
    )
...
def test_context_manager_signature_compatibility(context_manager):
    """Verify both legacy and new add_incident signatures."""
    ...
    context_manager.add_incident(
        ...
        "CRITICAL",
        "South Plaza",
        "Crowd backing up outside entry gate",
    )
    ...
    assert drone["title"] == "Unauthorized Drone Spotted"
    assert drone["priority"] == "HIGH"
    assert drone["area"] == "South Stand Skyspace"


def test_context_manager_alert_deduplication(context_manager):
    """Verify duplicate alerts are ignored."""
...
def test_context_manager_timeline_and_snaps(context_manager):
    """Verify timeline and telemetry snapshot limits."""
    ...
    for i in range(75):
        context_manager.add_telemetry_snapshot(
            {"data": i}
        )
...
def test_context_manager_command_status_and_risk(context_manager):
    """Verify command history and risk tracking."""
    ...
    assert (
        state["command_history"]["CMD-01"]["status"]
        == "EXECUTED"
    )

    context_manager.add_risk_score(
        42.5,
        "Heavy congestion risk",
    )
...
def test_context_manager_thread_safety(context_manager):
    """Verify concurrent operations remain thread-safe."""
    ...
    def worker(i):
        ...
        context_manager.add_timeline_event(
            f"Timeline concurrently added {i}"
        )
    ...
    # 20 manual timeline events + 20 incident timeline events
